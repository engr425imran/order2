...
@section('content')
    ...
    <div class="content-header row">
        <div class="content-header-left col-md-6 col-12 mb-2">
            <h3 class="content-header-title mb-0">View Invoice</h3>
            <div class="row breadcrumbs-top">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{URL::to('/')}}">Home</a>
                        </li>
                        <li class="breadcrumb-item"><a href="{{route('invoices')}}">Invoices List</a>
                        </li>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <table>
                                        <tr>
                                            ...
                                            <th class="pr-1"><label>Invoice ID</label></th>
                                        </tr>
                                        <tr>
                                            <td class="pr-1 color-1">{{$inv->display_name}}</td>
                                            <td class="pr-1">{{$inv->invoice_date}}</td>
                                            <td class="pr-1">{{$inv->due_date}}</td>
                                            <td class="pr-1">{{$invs->inv_name}}{{$inv->invoice_code}}</td>
                                        </tr>
                                    ...
                                    <table>
                                        <tr>
                                            ...
                                            <th class="pr-1"><label>Description</label></th>
                                        </tr>
                                        <tr>
                                            <td class="pr-1 color-1">{{$inv->c_street .' '. $inv->c_state .' '. $inv->c_city .' '. $inv->c_postal .' '. $inv->c_country}}</td>
                                            <td class="pr-1 color-1">{{$inv->description}}</td>
                                        </tr>
                                    </table>
                                    ...
                                    <table>
                                        @foreach ($inv_more_fields as $m_f)
                                            <tr >
                                                <th class="pr-1"><label>{{$m_f->new_label_name}}</label></th>
                                                <td class="pr-1 color-1">{{$m_f->new_field_data}}</td>
                                            </tr>
                                        @endforeach
                                            ...
                                            <tbody>
                                                @foreach ($inv_details as $item)
                                                    <tr class="content_data">
                                                        ...
                                                        <td class="w-15">
                                                            <div class="form-group form-group-sm">
                                                                <p>{{$item->tax_name.' ('.$item->tax_amount.'%)'}} </p>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <div class="form-group form-group-sm">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-10 text-right">
                                                <label class="mt-9 vat">Total Sales Tax </label>
                                        </div>
                                ...
                                <div class="row view_inv">
                                    <div class="col-md-8">
                                        <table>
                                            ...
                                            <tr>
                                                <form action="{{route('savePaymentInvoice')}}" method="post">
                                                    @csrf
            ...
            <div class="modal-content">
                ...
                <div id="doc_bill">
                    ...
                    <div class="form-group text-center">
                        <button type="button" class="btn c_btn pdf" onclick="genPDF()">Print PDF</button>
    ...
    <div class="hide">
        <form id="pdf_form">
            ...
            <input type="hidden" name="email" id="invoice_email">
        </form>
    </div>
...
@push('script')
                    ...
                    content: '' +
                        '<form class="formName">' +
                            '<div class="form-group">' +
                                ...
                                '<input type="email" placeholder="Email" class="email form-control" value={{$inv->email == '' ? $inv->customer->email : $inv->email}} required />' +
                            '</div>' +
                        '</form>',
                    buttons: {     
                        formSubmit: {
                            ...
                            action: function () {
                                ...
                                form = '<form id="pdf_email" class="formName hide">' +
                                    '<div class="form-group">' +
                                        '<label>Enter a valid email</label>' +
                                        '<input type="email" placeholder="Email" class="email form-control" name="email" value="'+email+'" required />' +
    ...
    <script type="text/javascript">
        function genPDF()
        {
            ...
            if (status == 1) {
                $.ajax({
                    url: "{{ route('invoice-status', $this_id) }}",
                    type: 'get',
                    success: function(resp){
                        if(resp){
                            createinvoicePDF('1');
                ...
                $.ajax({
                    url: "{{ route('update.invoice.print', $this_id) }}",
                    type: 'get',
                    success: function(resp){
                        if(resp){
                            createinvoicePDF('1');
        ...
        function createinvoicePDF(save){
            var invoice_name = $('.inv_name').val();
